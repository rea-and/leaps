...
public class LeapsWidget extends AppWidgetProvider {
  ...
  static void render(Context c, AppWidgetManager m, int id, String status){ RemoteViews v=new RemoteViews(c.getPackageName(),R.layout.leaps_widget); v.setTextViewText(R.id.status,status); v.setOnClickPendingIntent(R.id.medium,pending(c,"medium",id)); v.setOnClickPendingIntent(R.id.pills,pending(c,"pills",id)); m.updateAppWidget(id,v); }
